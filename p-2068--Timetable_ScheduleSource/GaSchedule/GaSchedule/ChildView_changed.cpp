...
const int ROOM_CELL_WIDTH = 150;
const int ROOM_CELL_HEIGHT = 50;

const int ROOM_MARGIN_WIDTH = 50;
const int ROOM_MARGIN_HEIGHT = 50;

const int ROOM_COLUMN_NUMBER = DAY_HOURS + 1;
const int ROOM_ROW_NUMBER = DAYS_NUM + 1;
		...
		for( int i = 0; i < ROOM_COLUMN_NUMBER; i++ )
		{
			for( int j = 0; j < ROOM_ROW_NUMBER; j++ )
			{
				...
				if( i == 0 && j == 0 )
				{
					...
					str.Format( "Lab: %s", Configuration::GetInstance().GetRoomById( k )->IsLab() ? "YES" : "NO" );
					dc.DrawText( str, rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE );
				...
				{
					CString str;
					str.Format( "%d:30 - %d:30", 7 + j - 1, 7 + j );
